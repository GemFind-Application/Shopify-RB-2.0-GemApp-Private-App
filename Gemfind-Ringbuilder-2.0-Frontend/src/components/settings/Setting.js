...
const Setting = (props) => {
    ...
    const searchStyle = new URLSearchParams(search).get("style");
    const searchMetalType = new URLSearchParams(search).get("metal");
    ...
    const getSettingProductsData = async (gettabname) => {
        ...
        // console.log(getpriceRange[0].maxPrice);
        // console.log(maxPrice);
        try {
            //console.log(gettabname);
            if (gettabname === "mined") {
    ...
    useEffect(() => {
        ...
        if (searchshape) {
            shapeselected(CapitalizeFirstLetter(searchshape));
        }
        if (searchStyle) {
            collectionselected(CapitalizeFirstLetter(searchStyle));
        }
        if (searchMetalType) {
            metalselected(CapitalizeFirstLetter(searchMetalType));
        }
    }, []);
    ...
    if (skeltonLoad === false) {
        ...
        return (
            ...
            .diamond-filter .navigation_filter_left .n_filter_left li.active a , .diamond-filter .navigation_filter_left .n_filter_left li.active span i{
